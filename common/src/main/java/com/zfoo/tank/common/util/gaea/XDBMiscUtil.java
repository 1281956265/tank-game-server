...
package com.zfoo.tank.common.util.gaea;
...
//	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T extends Weight> T rollWithWeight(Collection<? extends Weight> weights) {
		...
		// 随机获取该权重值
		int random = getRandom().nextInt(weightTotal + 1);
		weightTotal = 0;
		for (Weight weight : weights) {
			// 遍历列表，并累积元素的权重值
			weightTotal += weight.getWeightVal();
